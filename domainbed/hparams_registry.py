...
def _hparams(algorithm, dataset, random_seed):
    ...
    if algorithm in ['PathDANN', 'DANN', 'CDANN']:
    ...
    if algorithm == 'PIC':
        if dataset == 'TerraIncognita':
            _hparam('lr', 2.0e-3, lambda r: 2.0e-3)
        else:
            _hparam('lr', 1.5e-3, lambda r: 10**r.uniform(-3.0, -2.5))
        # _hparam('lr', 1.5e-3, lambda r: 10**r.uniform(-3.0, -2.5))
    ...
    if algorithm in ['PathDANN', 'DANN', 'CDANN'] and dataset in SMALL_IMAGES:
        _hparam('lr_g', 1e-3, lambda r: 10**r.uniform(-4.5, -2.5))
    elif algorithm in ['PathDANN', 'DANN', 'CDANN']:
