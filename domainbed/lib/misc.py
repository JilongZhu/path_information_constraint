...
class MovingAverage:
    ...
    def update(self, dict_data):
        ...
        for name, data in dict_data.items():
            name = name[len('refer_network.'):] if name.startswith('refer_network') else name
            # data = data.view(1, -1)
            if self._updates == 0:
...
def pred_aver_acc(models, loader, weights, device):
    correct = 0
    total = 0
    weights_offset = 0
...
def get_MovingAverageModel(models):
    weight_aver_dict = MovingAverage(ema=0.5, oneminusema_correction=False)
    for model in models:
        weight_aver_dict.update(model.state_dict())
    weight_aver_network = copy.deepcopy(models[0])
    weight_aver_network.load_state_dict(weight_aver_dict.ema_data)
    return weight_aver_network
